...
class DeliveryCarrier(models.Model):
    ...
    shipstation_package_id = fields.Many2one('product.packaging', string="Package")
    ...
    def shipstation_ts_rate_shipment(self, order):
        ...
        try:
            ...
            data = {
                ...
                "packageCode": self.shipstation_package_id.shipper_package_code,
            ...
            if package_code == 'package' or not package_code:
                data.update({"packageCode": 'package',
                             "dimensions":
                                 {
                                     "units": "inches",
                                     "height": self.shipstation_package_id.height,
                                     "length": self.shipstation_package_id.length,
    ...
    def shipstation_ts_send_shipping(self, pickings):
        ...
        for picking in pickings:
            ...
            total_bulk_weight = self.convert_ss_product_weight(total_bulk_weight)
            ...
            pick_attachments = []
            pick_messages = 'Shipment created!'
            exact_prices = 0.0
            carrier_trackings = ''
            ss_shipment_ids = ''
            for pack in picking.package_ids:
                package_code = pack.packaging_id.shipper_package_code
                data.update({"weight": {
                    "value": self.convert_ss_product_weight(pack.shipping_weight),
                    "units": 'ounces'
                }})
                if package_code == 'package' or not package_code:
                    data.update({"packageCode": 'package',
                ...
                try:
                    if picking.sale_id and picking.sale_id.shipstation_order_id:
                        ...
                        response = self.shipstation_account_id._send_request('shipments/createlabel', data,
                                                                             method="POST")
                except Exception as e:
                ...
                pick_messages += "<br/> <b>Tracking Number : </b>%s" % carrier_tracking_ref
                pick_attachments += [('Shipstation-%s.%s' % (pack.name, "pdf"), binary_label_data)]
                exact_prices += exact_price
                carrier_trackings = carrier_trackings and carrier_trackings + ',' + carrier_tracking_ref or carrier_tracking_ref
                ss_shipment_ids = ss_shipment_ids and ss_shipment_ids + ',' + str(ss_shipment_id) or str(ss_shipment_id)
            if not picking.package_ids:
                ...
                binary_label_data = binascii.a2b_base64(str(binary_label_data))
                pick_messages += "<br/> <b>Tracking Number : </b>%s" % carrier_tracking_ref
                pick_attachments += [('Shipstation-%s.%s' % (carrier_tracking_ref, "pdf"), binary_label_data)]
                exact_prices += exact_price
                carrier_trackings = carrier_trackings and carrier_trackings + ',' + carrier_tracking_ref or carrier_tracking_ref
                # if ss_shipment_ids:
                ss_shipment_ids = ss_shipment_ids and ss_shipment_ids + ',' + str(ss_shipment_id) or str(ss_shipment_id)
            shipping_data = {
                'exact_price': exact_prices,
                'tracking_number': carrier_trackings}
            res = res + [shipping_data]
            picking.write({'shipstation_shipment_id': ss_shipment_ids})
            picking.message_post(body=pick_messages, attachments=pick_attachments)
        return res

    def shipstation_ts_cancel_shipment(self, picking):
        try:
            for shipmentId in picking.shipstation_shipment_id.split(','):
                data = {'shipmentId': int(shipmentId)}
